...
async def _stream_message(client, **kwargs) -> anthropic.types.Message:
    """统一流式调用，解决 SDK 对长请求强制要求 streaming 的限制"""
...
此外，请用至少一轮搜索专门覆盖此人的**关系网络**：
- 投资人/合作伙伴关系（如"此人名 + 投资"、"此人名 + 融资"、"此人名 + 合作"）
- 竞争对手或同行的公开评价、交集、冲突（如"此人名 + 竞争"、"此人名 + 争议"、"此人名 + 回应"）
- 前同事、前合伙人的评价或相关报道
...
## 矛盾、沉默与未挖掘方向
- 此人在不同场合说过矛盾的话？（附原文出处）
- **沉默线索**：在早期采访或演讲中提到过、但后来再也没提及的项目、人物、观点或计划是什么？列出这些"消失的话题"及最后一次提及的出处。这些沉默本身就是最值得追问的线索。
- 采访者没有跟进但值得深挖的话题？
- 关系网络中的值得追问的点？（投资人、竞争对手、前同事相关的未解疑问）
...
async def analyze_single_video(
    ...
    try:
        message = await _stream_message(client,
...
每条记录必须包含完整的可点击URL链接。"""
    ...
    async def _run_round(prompt, round_num):
        try:
            response = await _stream_message(client,
