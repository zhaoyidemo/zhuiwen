...
async def search_douyin_videos(keyword: str, count: int = 20) -> list[dict]:
    """搜索抖音视频，返回简化的视频数据列表"""
    try:
        data = await _request("GET", "/api/v1/douyin/web/fetch_general_search_result", params={
            "keyword": keyword,
            "offset": 0,
            "count": min(count, 20),
            "sort_type": 0,
        ...
        for item in items:
            # 搜索结果可能包含多种类型（视频、用户、直播等）
            aweme_info = item.get("aweme_info")
            if not aweme_info:
                # 尝试从 aweme_mix_info 或直接作为视频数据
                aweme_info = item.get("aweme_mix_info", {}).get("mix_items", [{}])[0] if item.get("aweme_mix_info") else item
        ...
        logger.info(f"抖音搜索「{keyword}」: 找到 {len(results)} 条视频")
        return results
    except Exception as e:
        logger.warning(f"抖音搜索失败「{keyword}」: {e}")
        return []
